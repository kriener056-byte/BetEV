...
export default function Game() {
  ...
  const { oddsFormat, bankroll, kelly } = useSettings();
  ...
  return (
    <section className="mx-auto max-w-5xl">
      ...
      {/* Props with EV + Kelly suggestion */}
      <div className="mt-6">
        ...
        <div className="mt-2 grid gap-3 sm:grid-cols-2">
          {data.props.map((p, i) => {
            ...
            const kOverFull = kellyFraction(p.overOdds, nvOver);
            const kUnderFull = kellyFraction(p.underOdds, nvUnder);
            const kOverAdj = Math.max(0, kOverFull * kelly);
            const kUnderAdj = Math.max(0, kUnderFull * kelly);
            const stakeOver = Math.round(bankroll * kOverAdj);
            const stakeUnder = Math.round(bankroll * kUnderAdj);
              ...
              <div key={i} className="rounded-xl border bg-white p-4">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-3">
                  ...
                  <span className="text-xs text-slate-600">
                    {kOverFull <= 0 ? "Kelly —" : `Kelly ${(kOverAdj * 100).toFixed(1)}% · $${stakeOver}`}
                  </span>
                  ...
                  <span className="text-xs text-slate-600">
                    {kUnderFull <= 0 ? "Kelly —" : `Kelly ${(kUnderAdj * 100).toFixed(1)}% · $${stakeUnder}`}
                  </span>
                </div>
              </div>
            );
